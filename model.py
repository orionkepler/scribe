...
class Model:
    def __init__(self, args, logger):
        ...
        cell_func = tf.keras.layers.LSTM  # could be GRUCell or RNNCell
        self.cell0 = cell_func(args.rnn_size, kernel_initializer=self.graves_initializer, return_sequences=True, return_state=True)
        self.cell1 = cell_func(args.rnn_size, kernel_initializer=self.graves_initializer, return_sequences=True, return_state=True)
        self.cell2 = cell_func(args.rnn_size, kernel_initializer=self.graves_initializer, return_sequences=True, return_state=True)

        if self.train and self.dropout < 1:  # training mode
            self.cell0 = tf.compat.v1.nn.rnn_cell.DropoutWrapper(self.cell0, output_keep_prob=self.dropout)
            self.cell1 = tf.compat.v1.nn.rnn_cell.DropoutWrapper(self.cell1, output_keep_prob=self.dropout)
            self.cell2 = tf.compat.v1.nn.rnn_cell.DropoutWrapper(self.cell2, output_keep_prob=self.dropout)
